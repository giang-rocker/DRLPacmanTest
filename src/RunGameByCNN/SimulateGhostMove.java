 ...
 * @author Giang
 */
public class SimulateGhostMove  {
       private EnumMap<GHOST, POCommGhost> ghosts = new EnumMap<GHOST, POCommGhost>(GHOST.class);
    private EnumMap<GHOST, MOVE> myMoves = new EnumMap<GHOST, MOVE>(GHOST.class);

    public SimulateGhostMove() {
        this(5);
    }

    public SimulateGhostMove(int TICK_THRESHOLD){
        ghosts.put(GHOST.BLINKY, new POCommGhost(GHOST.BLINKY, TICK_THRESHOLD));
        ghosts.put(GHOST.INKY, new POCommGhost(GHOST.INKY, TICK_THRESHOLD));
        ghosts.put(GHOST.PINKY, new POCommGhost(GHOST.PINKY, TICK_THRESHOLD));
        ghosts.put(GHOST.SUE, new POCommGhost(GHOST.SUE, TICK_THRESHOLD));
    }

    
    public EnumMap<GHOST, MOVE> getMove(Game game ) {
        myMoves.clear();
        for (GHOST ghost : ghosts.keySet()) {
            MOVE move = ghosts.get(ghost).getMove(game.copy(ghost) );
            if (move != null) {
